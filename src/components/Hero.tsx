...
const Hero: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      <div className="text-hero font-bold text-[3.5rem] leading-[1.2]">
  ...
  <br />
  {'RAJ'.split('').map((letter, index) => (
    <span key={`raj-${index}`} className="hero-letter inline-block">
      {letter}
    </span>
  ))}
  <br />
  {'SHIVHARE'.split('').map((letter, index) => (
